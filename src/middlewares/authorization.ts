...
export const authenticateTokenAdmin: (req: Request, res: Response, next: NextFunction) => Promise<void> = async (
  ...
  try {
    ...
    const user = await UsersModel.query().findById(tokenPayload.id).whereIn('role', ['superadmin', 'admin'])
    if (user === undefined) {
      ResponseHelper.error('Email or password invalid', null, 401)(res)
      return
    }
    const isHavetoken = await UsersModel.query().whereIn('role', ['superadmin', 'admin'])
